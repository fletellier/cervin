...
public class OAuth2Client {
	...
	public static void main(String[] args) {
		...
		if (!OAuthUtils.isValid(username)){
			System.out.println("Please provide valid values for username");
			System.exit(0);
		}
		if (!OAuthUtils.isValid(password)){
			System.out.println("Please provide valid values for password");
			System.exit(0);
		}
		if (!OAuthUtils.isValid(authenticationServerUrl)){
			System.out.println("Please provide valid values for authenticationServerUrl");
			System.exit(0);
		}
		if (!OAuthUtils.isValid(grantType)){
			System.out.println("Please provide valid values for grantType");
			System.exit(0);
		}
